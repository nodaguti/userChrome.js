...
## 概要
   ...
   * ブロックしたURLはエラーコンソールに出力されます.
   ...
   本スクリプトはFirefox 3.6 - Firefox 15対応としているが, 基本的にFirefox 3.6.28, Firefox 10 ESR, 最新安定版(Releaseチャンネル)のFirefox,
...
### adblock#.uc.js
   ...
   Filter Manager搭載のバージョン. 通常はこちらを推奨.
   ただ, $third-party や $domain へは対応していない.
   なお, この項目以降の説明は基本的にこのバージョンについての説明となっています.

### adblock#2.uc.js

   通常の adblock#.uc.js に $third-party と $domain への対応を追加したバージョン.
   マッチ処理が全体的に10倍程度遅くなってしまった($domainなどを未使用の場合でも)が, フィルタの数が多くなければそれほど気にならないレベル.
   Adblock Plusなどとの更なる互換性を求める方はこちらをご使用下さい.

### adblock#_lite.uc.js

   adblock#.uc.js 10/05/21 21:30 相当バージョン (observerバグ修正済み).
   最も動作が軽いバージョンだが, その分機能は限られる.
   Filter Managerはなく, フィルタはスクリプト内の filter: の部分に直接追加する.
