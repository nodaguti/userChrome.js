...
adblockSharp.filterManager = {
	...
	loadFilter: function(){
		try{
			...
			for(let i=0,l=this.blackList.regFilter.length;i<l;i++){
				var regStr = this.blackList.regFilter[i], re = new RegExp('');
				re.compile(regStr.slice(1, regStr.length-1));
		...
		try{
			...
			for(let i=0,l=this.whiteList.regFilter.length;i<l;i++){
				var regStr = this.whiteList.regFilter[i], re = new RegExp('');
				re.compile(regStr.slice(1, regStr.length-1));
	...
	optimize: function(doc){
		...
		function compare(a, b){
			var aData = adblockSharp.filterManager.storage['result'].getItem(a+''),
				...
				aTime = aData && (aData = aData.value) ? aData.slice(aData.indexOf('time')+6, aData.lastIndexOf('}')) : 0,
				bTime = bData && (bData = bData.value) ? bData.slice(bData.indexOf('time')+6, bData.lastIndexOf('}')) : 0,
				aDate = aData ? aData.slice(aData.indexOf('date')+6, aData.lastIndexOf(',"time"')) : 0,
				bDate = bData ? bData.slice(bData.indexOf('date')+6, bData.lastIndexOf(',"time"')) : 0;
	...
	setResult: function(url, filter){
		...
		var storage = this.storage['result'],
			data = storage.getItem(filter), str = data && data.value,
			time = typeof str === 'string' ? str.slice(str.indexOf('time')+6, str.lastIndexOf('}')) - (-1) : 1;
		...
		function getResult(filterset, filtername, kind, listKind){
			...
			for(let i=0;i<len;i++){
				...
				if(jsonData){
					data = jsonData.value;
					str += '<tr><td>'+s+'</td>'
							+'<td>'+(new Date(data.slice(data.indexOf('date')+6, data.lastIndexOf(',"time"'))-0)).toLocaleString()+'</td>'
							+'<td>'+(data.slice(data.indexOf('time')+6, data.lastIndexOf('}')))+'</td>'
...
adblockSharp.observer = {
	start: function(){
		var flag = window.globalStorage['adblockSharp.started'].getItem('flag');
		if(!flag){
			...
			window.globalStorage['adblockSharp.started'].setItem('flag', 'true');
	...
	stop: function(){
		...
		if(this.observer || isLastWindow){
			window.globalStorage['adblockSharp.started'].removeItem('flag');
